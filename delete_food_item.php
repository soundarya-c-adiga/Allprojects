...
<input type="text" class="form-control" name="fname" placeholder="Enter the food name you want to delete" required>
</fieldset>
<fieldset>
<button type="submit" class="btn btn-primary" name="submit">Delete food item</button>
</fieldset>
...
{
	$name=$_POST['fname'];
	$nquery="select * from `foodlist` where `name`='$name'";
	$nresult=mysqli_query($link,$nquery);
	if(mysqli_num_rows($nresult)==0)
	{
		?> <p>These is no such food.</p> <?php
	}
	else
	{
	...
	{
		?> <p>Successfully deleted</p> <?php
	}else{
		?> <p>Delete was not possible </p> <?php
	}
	}
}
?>	  